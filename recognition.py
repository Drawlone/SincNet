...
model_file = 'E:\\Project\\python\\voiceprint\\SincNet\\sincnet_models\\SincNet_TIMIT\\model_raw.pkl'  # This is the model to use for computing the d-vectors (it should be pre-trained using the speaker-id DNN)
cfg_file = 'E:\\Project\python\\voiceprint\\SincNet\\cfg\\SincNet_TIMIT.cfg'  # Config file of the speaker-id experiment used to generate the model
...
def d_vector(audio_path):
    with torch.no_grad():
        ...
        return d_vect_out.cpu().numpy()


def load_audio(audio_path, wlen):
    [signal, fs] = sf.read(audio_path)
    ...
    if channels == 2:
        ...
        signal = signal[:, 0]
    inp = torch.tensor(signal[snt_beg:snt_end], dtype=torch.float32, device=device)
    return inp[np.newaxis, :]


def get_features(inp):
    feature = DNN2_net(DNN1_net(CNN_net(inp)))
    return feature.data.cpu().numpy()


audio1_path = "audio/a_1.wav"
audio2_path = "audio/c_2.wav"
# audio1 = load_audio(audio1_path, wlen)
# audio2 = load_audio(audio2_path, wlen)
# feature1 = get_features(audio1)[0]
# feature2 = get_features(audio2)[0]
# dist = np.dot(feature1, feature2) / (np.linalg.norm(feature1) * np.linalg.norm(feature2))
d1 = d_vector(audio1_path)[np.newaxis, :]
d2 = d_vector(audio2_path)[np.newaxis, :]
simi = cosine_similarity(d1, d2)
print(simi)
